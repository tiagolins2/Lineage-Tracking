...
folder_location = input("Enter the full path of the folder containing data: ")


output1 = os.path.join(folder_location, 'output.csv')
...
output2 = os.path.join(folder_location, 'output_paths.txt')
...
model_fd = YOLO("combinedbest.pt"); # a slightly improved model
...
def run_tracking_model(path, model_fd, model, device, timestep_value, timestep_unit, radial_threshold, C_params=C_params, C_iou=2, C_d=0.1, C_iou_d=1, Ci_iou=20, C_siam=10, C_f=1):
  ...
  for id_, addresses_info in addresses_by_id.items():
        ...
        list_files.sort();
        #print(list_files)
        global_id = np.ones([100,600+2])*-1 #max of 200 frames
        global_id_output = np.empty((101, 200), dtype='object')#np.ones([101,100+2])*-1
        ...
        for i,filename in enumerate(list_files):
          ...
          if i==0:
            ...
            for jj in range(len(centroids_set1)):
              ...
              plt.text(centroids_set1[jj][0],centroids_set1[jj][1],f"{idx[0][0]}",horizontalalignment='center')
           ...
           # write_matrix_indices(matrix2, budding_indices_file, 2)
            ...
            ## add new ids for new daughter fronds:
             ...
             if len(set1)>0:
               for mset1, mset2 in zip(set1, set2):
                 ...
                 global_id_output[new_element+1,i] = f"{new_element} - {x_mother[0][0]}";   
            ...
            ##plotting
             ...
             for jj in range(len(centroids_set2)):
               idx = np.where(global_id[:,2+(i)*3]==jj);
               if idx[0].size>0:
                 plt.text(centroids_set2[jj][0],centroids_set2[jj][1],f"{idx[0][0]}",horizontalalignment='center') #the original with global ids
               #plt.text(centroids_set2[jj][0],centroids_set2[jj][1],f"{jj}",horizontalalignment='center')
             plt.imshow(img2plot);
             plt.axis("off")
             plt.savefig(saving_address+"/"+f'plot_{i}.png')
             plt.show()
...
path = folder_location;#input("Enter the full path of the directory containing addresses.txt: ")
